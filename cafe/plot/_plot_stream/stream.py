...
def assign_root(adata, root="root", tree="stream_tree"):
...
def plot_stream_sc(
    ...
    df_plot = pd.DataFrame(index=adata.obs.index, data=adata.obsm["X_stream_" + root], columns=["pseudotime", "dist"])
    for ann in color:
    ...
    if plotly:
        for ann in color:
            ...
            if show_text:
                fig.add_trace(
                    go.Scatter(
                        x=np.array(list(stream_nodes.values()))[:, 0],
        ...
        for i, ann in enumerate(tqdm_show(color, desc="stream sc plotting")):
            ...
            if is_string_dtype(df_plot[ann]):  ## plot groups
                ...
                if (isinstance(s, list) or isinstance(s, tuple)) and len(s) == 2:  ## variated size of point size range e.g. (2, 50)
                    s_min = s[0]
                    s_max = s[1]
                    df_plot_shuf["weight"] = s_max * (
                        (df_plot_shuf[ann] - min(df_plot_shuf[ann])) / (max(df_plot_shuf[ann]) - min(df_plot_shuf[ann]))
                    )
                    ...
                    if order:
                        ...
                        for a_cut in set(df_plot_shuf["zorder"]):
                            ...
                            if df_plot_shuf_i.empty:
                                continue
                            sc_i = ax_i.scatter(
                                df_plot_shuf_i["pseudotime"],
                                df_plot_shuf_i["dist"],
                                c=df_plot_shuf_i[ann],
                                cmap=cmap_obj,
                                vmin=vmin_i,
                                vmax=vmax_i,
                                alpha=alpha,
                        ...
                        sc_i = ax_i.scatter(
                            list(df_plot_shuf["pseudotime"]),
                            list(df_plot_shuf["dist"]),
                            s=list(df_plot_shuf["weight"]),
                            c=list(df_plot_shuf[ann]),
                            vmin=vmin_i,
                            vmax=vmax_i,
                            alpha=alpha,
                            cmap=cmap_obj,
                        )  # , zorder=[int(i*10) for i in df_plot_shuf['weight']])
            ...
            if show_graph:
                for edge_i in stream_edges.keys():
                    ...
                    for ii in np.arange(start=0, stop=branch_i.shape[0], step=2):
                        if branch_i.iloc[ii, 0] == branch_i.iloc[ii + 1, 0]:
                            ax_i.plot(branch_i.iloc[[ii, ii + 1], 0], branch_i.iloc[[ii, ii + 1], 1], c="#767070", alpha=0.8)
                        else:
                            ax_i.plot(branch_i.iloc[[ii, ii + 1], 0], branch_i.iloc[[ii, ii + 1], 1], c="black", alpha=1)
            if show_text:
                for node_i in stream_tree.nodes():
                    ax_i.text(
                        stream_nodes[node_i][0],
                        stream_nodes[node_i][1],
                        ft_node_label[node_i] if not dd else str(dd.get(ft_node_label[node_i], ft_node_label[node_i])),
                        color="black",
            ...
            annots = arrowed_spines(ax_i, locations=("bottom right",), lw=ax_i.spines["bottom"].get_linewidth() * 1e-5)
            title = ""
            if titles is None:
                title = ann
            else:
...
def plot_stream(
    ...
    if len(list_string_type) > 0:
        dict_verts, dict_extent = cal_stream_polygon_string(
            adata,
            dict_ann,
            root=root,
    ...
    if len(list_numeric_type) > 0:
        verts, extent, ann_order, dict_ann_df, dict_im_array = cal_stream_polygon_numeric(
            adata,
            dict_ann,
            root=root,
            preference=preference,
    ...
    for i, ann in enumerate(tqdm_show(color, desc="stream plotting")):
        if is_string_dtype(dict_ann[ann]):
            if not ((ann + "_color" in adata.uns_keys()) and (set(adata.uns[ann + "_color"].keys()) >= set(np.unique(dict_ann[ann])))):
                ...
                tmp = pd.DataFrame(index=adata.obs_names, data=np.random.rand(adata.shape[0], 2))
        ...
        annots = arrowed_spines(ax, locations=("bottom right",), lw=ax.spines["bottom"].get_linewidth() * 1e-5)
        title = ""
        if titles is None:
            title = ann
        else:
    ...
    for edge_i in bfs_edges:
        ...
        for markername in input_markers_expressed:
            ...
            if diff_initial_final > 0:
                logfc = np.log2(
                    max(values_final.mean(), values_initial.mean()) / (min(values_final.mean(), values_initial.mean()) + diff_initial_final / 1000.0)
                )
            else:
                logfc = 0
            if logfc > cutoff_logfc:
                df_stat_pval_qval.loc[markername] = np.nan
                df_stat_pval_qval.loc[markername, ["stat", "pval"]] = spearmanr(
                    df_cells_edge_i_sort.loc[:, markername], df_cells_edge_i_sort.loc[:, "lam_ordered"]
                )
        ...
        if df_stat_pval_qval.shape[0] == 0:
            ...
            dict_tg_edges[edge_i_alias].to_csv(
                os.path.join(file_path, "transition_markers_" + edge_i_alias[0] + "_" + edge_i_alias[1] + ".tsv"), sep="\t", index=True
            )
    adata.uns["transition_markers"] = dict_tg_edges


def plot_transition_markers(
    adata,
    num_markers=15,
    save_fig=False,
    fig_path=None,
    ...
    for edge_i in dict_tg_edges.keys():
        ...
        for i, rect in enumerate(rects):
            if stat[i] > 0:
                ...
                ax.text(rect.get_x() + rect.get_width() + 0.01, rect.get_y() + rect.get_height() / 2.0, qvals.index[i], **alignment)
                ax.text(
                    rect.get_x() + 0.01,
                    rect.get_y() + rect.get_height() / 2.0,
                    "{:.2E}".format(Decimal(str(qvals[i]))),
                    size=0.8 * mpl.rcParams["font.size"],
                    color="black",
                    **alignment,
                )
            else:
        ...
        plt.tight_layout()
        if save_fig:
            plt.savefig(os.path.join(fig_path, "transition_markers_" + edge_i0 + "_" + edge_i1 + ".pdf"), pad_inches=1, bbox_inches="tight")
            plt.close(fig)


def detect_leaf_markers(
    adata,
    marker_list=None,
    cutoff_zscore=1.0,
    cutoff_pvalue=1e-2,
        ...
        if leaf_mean_values.shape[0] <= 2:
            ...
            if abs(zscores)[abs(zscores) > cutoff_zscore].shape[0] >= 1:
                ...
                if kurskal_pvalue < cutoff_pvalue:
                    df_conover_pvalues = posthoc_conover(
                        df_marker_detection[[x in leaf_edges for x in df_marker_detection["bfs_edges"]]],
                        val_col=marker,
                        group_col="bfs_edges",
                        p_adjust="fdr_bh",
                    )

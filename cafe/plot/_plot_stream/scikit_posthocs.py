...
def __convert_to_block_df(a, y_col, group_col, block_col, melted):
    if isinstance(a, DataFrame) and not melted:
        ...
        x = DataFrame.from_dict({"groups": a[group_col], "blocks": a[block_col], "y": a[y_col]})
...
def posthoc_conover(a, val_col=None, group_col=None, p_adjust=None, sort=True):
    ...
    def compare_conover(i, j):
        ...
        B = 1.0 / x_lens[i] + 1.0 / x_lens[j]
        D = (x_len_overall - 1.0 - H) / (x_len_overall - x_len)
        t_value = diff / np.sqrt(S2 * B * D)
        p_value = 2.0 * ss.t.sf(np.abs(t_value), df=x_len_overall - x_len)
        return p_value
    ...
    def get_ties(x):
        ...
        while pos < x_len_overall:
            ...
            pos = pos + n_ties
            if n_ties > 1:
                tie_sum += n_ties**3.0 - n_ties
        c = np.min([1.0, 1.0 - tie_sum / (x_len_overall**3.0 - x_len_overall)])
        return c
    ...
    if x_ties == 1:
        S2 = x_len_overall * (x_len_overall + 1.0) / 12.0
    else:
        S2 = (1.0 / (x_len_overall - 1.0)) * (np.sum(x_ranks**2.0) - (x_len_overall * (((x_len_overall + 1.0) ** 2.0) / 4.0)))
...
def posthoc_dunn(a, val_col=None, group_col=None, p_adjust=None, sort=True):
    ...
    def compare_dunn(i, j):
        diff = np.abs(x_ranks_avg[i] - x_ranks_avg[j])
        A = x_len_overall * (x_len_overall + 1.0) / 12.0
        B = 1.0 / x_lens[i] + 1.0 / x_lens[j]
    ...
    def get_ties(x):
        ...
        while pos < x_len_overall:
            ...
            if n_ties > 1:
                tie_sum += n_ties**3.0 - n_ties
        c = tie_sum / (12.0 * (x_len_overall - 1))
        return c
...
def posthoc_nemenyi(a, val_col=None, group_col=None, dist="chi", sort=True):
    ...
    def compare_stats_tukey(i, j):
        diff = np.abs(x_ranks_avg[i] - x_ranks_avg[j])
        B = 1.0 / x_lens[i] + 1.0 / x_lens[j]
        q = diff / np.sqrt((x_len_overall * (x_len_overall + 1.0) / 12.0) * B)
        return q
    ...
    if dist == "chi":
        ...
        for i, j in combs:
            vs[i, j] = compare_stats_tukey(i, j) * np.sqrt(2.0)
...
def posthoc_conover_friedman(a, y_col=None, block_col=None, group_col=None, melted=False, sort=False, p_adjust=None):
    ...
    def compare_stats(i, j):
        ...
        pval = 2.0 * ss.t.sf(np.abs(tval), df=(n - 1) * (k - 1))
        return pval
...
def posthoc_npm_test(a, y_col=None, group_col=None, sort=False, p_adjust=None):
    ...
    def compare(m, u):
        return (Ri[u] - Ri[m]) / (sigma / np.sqrt(2) * np.sqrt(1.0 / ni[m] + 1.0 / ni[u]))
...
def posthoc_scheffe(a, val_col=None, group_col=None, sort=False, p_adjust=None):
    ...
    xi = x_grouped.mean()
    si = x_grouped.var()
    sin = 1.0 / (n - x_groups_unique.size) * np.sum(si * (ni - 1.0))

    def compare(i, j):
        dif = xi[i] - xi[j]
        A = sin * (1.0 / ni[i] + 1.0 / ni[j]) * (x_groups_unique.size - 1.0)
        f_val = dif**2.0 / A
        return f_val
    ...
    vs[tri_lower] = vs.T[tri_lower]
    p_values = ss.f.sf(vs, x_groups_unique.size - 1.0, n - x_groups_unique.size)
...
def posthoc_tamhane(a, val_col=None, group_col=None, welch=True, sort=False):
    ...
    xi = x_grouped.mean()
    si = x_grouped.var()
    sin = 1.0 / (n - x_groups_unique.size) * np.sum(si * (ni - 1))
    ...
    def compare(i, j):
        ...
        t_val = dif / np.sqrt(A)
        if welch:
            df = A**2.0 / (si[i] ** 2.0 / (ni[i] ** 2.0 * (ni[i] - 1.0)) + si[j] ** 2.0 / (ni[j] ** 2.0 * (ni[j] - 1.0)))
        else:
            ## checks according to Tamhane (1979, p. 474)
            ok1 = (9.0 / 10.0 <= ni[i] / ni[j]) and (ni[i] / ni[j] <= 10.0 / 9.0)
            ok2 = (9.0 / 10.0 <= (s2i[i] / ni[i]) / (s2i[j] / ni[j])) and ((s2i[i] / ni[i]) / (s2i[j] / ni[j]) <= 10.0 / 9.0)
            ok3 = (
                (4.0 / 5.0 <= ni[i] / ni[j])
                and (ni[i] / ni[j] <= 5.0 / 4.0)
                and (1.0 / 2.0 <= (s2i[i] / ni[i]) / (s2i[j] / ni[j]))
                and ((s2i[i] / ni[i]) / (s2i[j] / ni[j]) <= 2.0)
            )
            ok4 = (
                (2.0 / 3.0 <= ni[i] / ni[j])
                and (ni[i] / ni[j] <= 3.0 / 2.0)
                and (3.0 / 4.0 <= (s2i[i] / ni[i]) / (s2i[j] / ni[j]))
                and ((s2i[i] / ni[i]) / (s2i[j] / ni[j]) <= 4.0 / 3.0)
            )
    ...
    vs[tri_upper] = 1.0 - (1.0 - vs[tri_upper]) ** x_groups_unique.size
    vs[tri_lower] = vs.T[tri_lower]
    vs[vs > 1] = 1

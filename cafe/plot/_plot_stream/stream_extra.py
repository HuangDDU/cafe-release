...
def arrowed_spines(ax, x_width_fraction=0.03, x_height_fraction=0.02, lw=None, ohg=0.2, locations=("bottom right", "left up"), **arrow_kwargs):
    """
    ...
    for loc_str in locations:
        ...
        annots[loc_str] = ax.arrow(x, y, dx, dy, fc="k", ec="k", width=lw, head_width=head_width, head_length=head_length, **arrow_kwargs)

    return annots


def cal_stream_polygon_numeric(
    adata,
    dict_ann,
    root="S0",
    preference=None,
    ...
    for x in bfs_edges:
        if x in nx.get_edge_attributes(stream_tree, "id").values():
            ...
            df_stream.loc[df_stream.index[id_cells], "edge"] = pd.Series(index=df_stream.index[id_cells], data=[x] * len(id_cells))
            ...
            df_stream.loc[df_stream.index[id_cells], "edge"] = pd.Series(index=df_stream.index[id_cells], data=[x] * len(id_cells))
            df_stream.loc[df_stream.index[id_cells], "lam_ordered"] = (
                stream_tree.edges[x]["len"] - df_stream.loc[df_stream.index[id_cells], "branch_lam"]
            )
    ...
    if (stream_tree.degree(root_node)) > 1:
        ...
        dict_shift_dist[(root_node, root_node)] = (sum([dict_shift_dist[(root_node, next_node)] for next_node in next_nodes])) / float(
            len(next_nodes)
        )
    ...
    for i, edge_i in enumerate(bfs_edges):
        ...
        mat_w = np.vstack(
            [
                np.arange(0, len_ori[edge_i] - size_w + (len_ori[edge_i] / 10**6), step_w),
                np.arange(size_w, len_ori[edge_i] + (len_ori[edge_i] / 10**6), step_w),
            ]
        ).T
        ...
        if degree_st > 1 and i == 0:
            ...
            for i_win in range(mat_w_common.shape[0]):
                ...
                for j in range(degree_st):
                    ...
                    cell_num_common2 = df_edge_j[np.logical_and(df_edge_j.lam_ordered >= 0, df_edge_j.lam_ordered <= mat_w_common[i_win, 1])][
                        "CELL_LABEL"
                    ].value_counts()
                    ...
                    for ann in list_ann_numeric:
                        dict_df_ann_common[ann].append(
                            df_edge_j[np.logical_and(df_edge_j.lam_ordered >= 0, df_edge_j.lam_ordered <= mat_w_common[i_win, 1])]
                        )
        ...
        if max_binnum <= 1:
            for i_win in range(mat_w.shape[0]):
                df_bins["win" + str(total_bins + i_win)] = ""
                df_bins.loc[df_bins.index[:-3], "win" + str(total_bins + i_win)] = 0
                cell_num = df_edge_i[np.logical_and(df_edge_i.lam_ordered >= mat_w[i_win, 0], df_edge_i.lam_ordered <= mat_w[i_win, 1])][
                    "CELL_LABEL"
                ].value_counts()
                ...
                for ann in list_ann_numeric:
                    ...
                    ann_values = (
                        df_edge_i[np.logical_and(df_edge_i.lam_ordered >= mat_w[i_win, 0], df_edge_i.lam_ordered <= mat_w[i_win, 1])]
                        .groupby(["CELL_LABEL"])[ann]
                        .mean()
                    )
            ...
            df_bins.loc["edge", col_wins] = pd.Series(index=col_wins, data=[[edge_i]] * len(col_wins))
        ...
        if max_binnum > 1:
            ...
            for i_win in range(mat_w.shape[0]):
                ...
                cell_num = df_edge_i[np.logical_and(df_edge_i.lam_ordered >= bd_bins[0], df_edge_i.lam_ordered <= bd_bins[1])][
                    "CELL_LABEL"
                ].value_counts()
                if len(id_stack) == max_binnum or any(cell_num > min_bin_cellnum) or i_win == mat_w.shape[0] - 1:
                    ...
                    for ann in list_ann_numeric:
                        ...
                        ann_values = (
                            df_edge_i[np.logical_and(df_edge_i.lam_ordered >= bd_bins[0], df_edge_i.lam_ordered <= bd_bins[1])]
                            .groupby(["CELL_LABEL"])[ann]
                            .mean()
                        )
        ...
        if degree_end > 1:
            ...
            mat_w_common = np.vstack(
                [
                    np.arange(len_ori[edge_i] - size_w + step_w, len_ori[edge_i] + (len_ori[edge_i] / 10**6), step_w),
                    np.arange(step_w, size_w + (len_ori[edge_i] / 10**6), step_w),
                ]
            ).T
            # neighbor nodes
            ...
            if mat_w_common.shape[0] > 0:
                for i_win in range(mat_w_common.shape[0]):
                    df_bins["win" + str(total_bins + i_win)] = ""
                    df_bins.loc[df_bins.index[:-3], "win" + str(total_bins + i_win)] = 0
                    cell_num_common1 = df_edge_i[
                        np.logical_and(df_edge_i.lam_ordered > mat_w_common[i_win, 0], df_edge_i.lam_ordered <= len_ori[edge_i])
                    ...
                    for ann in list_ann_numeric:
                        ...
                        dict_df_ann_common[ann] = list()
                        dict_df_ann_common[ann].append(
                            df_edge_i[np.logical_and(df_edge_i.lam_ordered > mat_w_common[i_win, 0], df_edge_i.lam_ordered <= len_ori[edge_i])]
                        )
    ...
    dict_forest = {cellname: {nodename: {"prev": "", "next": "", "div": ""} for nodename in bfs_nodes} for cellname in df_edge_cellnum.index}
    ...
    if len(id_wins_start) > 0:
        mean_shift_dist = np.mean([dict_shift_dist[(root_node, x)] for x in dict_forest[cell_list_sorted[0]][root_node]["next"]])
        for cellname in cell_list_sorted:
            # top line
    ...
    x_smooth = np.unique(
        np.arange(min(df_base_x.iloc[0, :]), max(df_base_x.iloc[0, :]), step=step_w / 20).tolist() + [max(df_base_x.iloc[0, :])]
    ).tolist()
    ...
    for edge_i_top in dict_paths_top.keys():
        ...
        if stream_tree.degree(root_node) > 1 and edge_i_top == (root_node, dict_forest[cell_list_sorted[0]][root_node]["next"][0]):
        ...
        for cellname in cell_list_sorted:
            ...
            for id_node in range(len(path_i_top) - 1):
                ...
                dict_smooth_linear[cellname]["top"][edge_i] = pd.DataFrame(
                    [np.array(x_top_new)[id_selected], np.array(y_top_new_linear)[id_selected]], index=["x", "y"]
                )
    for edge_i_base in dict_paths_base.keys():
        ...
        if stream_tree.degree(root_node) > 1 and edge_i_base == (root_node, dict_forest[cell_list_sorted[0]][root_node]["next"][-1]):
        ...
        for cellname in cell_list_sorted:
            ...
            for id_node in range(len(path_i_base) - 1):
                ...
                dict_smooth_linear[cellname]["base"][edge_i] = pd.DataFrame(
                    [np.array(x_base_new)[id_selected], np.array(y_base_new_linear)[id_selected]], index=["x", "y"]
                )
    ...
    for cellname in cell_list_sorted:
        for edge_i in bfs_edges:
            if sum(abs(dict_smooth_linear[cellname]["top"][edge_i].loc["y"] - dict_smooth_linear[cellname]["base"][edge_i].loc["y"]) > 1e-12):
    ...
    for cellname in cell_list_sorted:
        ...
        for edge_i_top in paths_CE_top.keys():
            ...
            for id_node in range(len(path_i_top) - 1):
                ...
                edge_i_bd = dict_edge_bd[edge_i]
                id_selected = [i_x for i_x, x in enumerate(x_top_new) if x >= edge_i_bd[0] and x <= edge_i_bd[1]]
                dict_smooth_new[cellname]["top"][edge_i] = pd.DataFrame(
                    [np.array(x_top_new)[id_selected], np.array(y_top_new)[id_selected]], index=["x", "y"]
                )
        ...
        for edge_i_base in paths_CE_base.keys():
            ...
            for id_node in range(len(path_i_base) - 1):
                ...
                edge_i_bd = dict_edge_bd[edge_i]
                id_selected = [i_x for i_x, x in enumerate(x_base_new) if x >= edge_i_bd[0] and x <= edge_i_bd[1]]
                dict_smooth_new[cellname]["base"][edge_i] = pd.DataFrame(
                    [np.array(x_base_new)[id_selected], np.array(y_base_new)[id_selected]], index=["x", "y"]
                )
    ...
    for x in range(len(id_y_base)):
        if x in range(int(step_w / xmax * im_ncol)) and prev_edge != "":
            if shift_dist > 0:
                ...
                id_y_top[x] = id_y_top[x] + int(im_ncol * pad_ratio) - int(abs(shift_dist) / abs(ymin - ymax) * im_nrow * 0.3)
                if id_y_top[x] < 0:
                    id_y_top[x] = 0
            if shift_dist < 0:
                id_y_base[x] = id_y_base[x] - int(im_ncol * pad_ratio) + int(abs(shift_dist) / abs(ymin - ymax) * im_nrow * 0.3)
                id_y_top[x] = id_y_top[x] + int(im_ncol * pad_ratio)
                if id_y_base[x] > im_nrow - 1:
                    id_y_base[x] = im_nrow - 1
        dict_im_array[cellname][id_y_top[x] : (id_y_base[x] + 1), id_x_base[x]] = np.tile(gene_color_new[x], (id_y_base[x] - id_y_top[x] + 1))

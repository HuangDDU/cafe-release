...
def calculate_position_predict(
    ...
    def _mat(w):
        ...
        mat = pd.pivot_table(
            df, index="cell_id", columns="milestone_id",
    ...
    train_idx, test_idx = train_test_split(
        gold.index, test_size=test_size, random_state=random_state
    )
    ...
    baseline_mses = [
        ((gold_test[col] - gold_test[col].mean())**2).mean()
        for col in gold_test
    ]
    ...
    if any(m in metrics for m in ("rf_mse","rf_rsq","rf_nmse")):
        ...
        for col in gold.columns:
            ...
            rf = RandomForestRegressor(
                n_estimators=2000, random_state=random_state, n_jobs=1
            )
            rf.fit(Xtr, ytr)
            pte = rf.predict(Xte)

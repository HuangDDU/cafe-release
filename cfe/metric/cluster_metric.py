...
def calculate_mapping(
    ...
    # 检查轨迹数据是否存在
    if not fadata_ref.uns.get("cfe", {}).get("trajectory_history_dict") or not fadata_pred.uns.get("cfe", {}).get("trajectory_history_dict"):
    ...
    # 计算Jaccard相似度矩阵
    jaccard_matrix = pd.DataFrame(index=groups_ref.index, columns=groups_pred.index, dtype=float)
...
def calculate_mapping_milestones(fadata_ref: FateAnnData, fadata_pred: FateAnnData, **kwargs) -> dict:
    """计算里程碑分组映射指标"""
...
def calculate_mapping_branches(fadata_ref: FateAnnData, fadata_pred: FateAnnData, **kwargs) -> dict:
    """计算分支分组映射指标"""

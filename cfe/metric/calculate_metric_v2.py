...
def calculate_metrics(
    ...
    ref_model: str,
    simplify: bool = True,
    metrics: list[str] = ["isomorphic", "edge_flip"]
) -> dict:
    """
    汇总多种轨迹对比指标。
    ...
    if "correlation" in metrics:
        corr_res = calculate_correlation(
            fadata,
            ref_model=ref_model,
            pred_model=now_model
        )
    ...
    if any(k in metrics for k in pos_keys):
        pos_res = calculate_position_predict(
            fadata,
            ref_model=ref_model,
            pred_model=now_model,
            metrics=metrics
    ...
    if any(k in metrics for k in fi_cor_keys):
        fic_res = calculate_featureimp_cor(
            fadata,
            ref_model=ref_model,
            pred_model=now_model
        )
    ...
    if any(k in metrics for k in fi_enr_keys):
        fie_res = calculate_featureimp_enrichment(
            fadata,
            ref_model=ref_model,
            pred_model=now_model
        )
    ...
    if "F1_milestone" in metrics:
        cm2 = calculate_mapping_milestones(
            fadata,
            simplify=simplify,
            ref_model=ref_model,
            pred_model=now_model

...
def infer(
    ...
    method = cfe.method.FateMethod(method_name=method)
    method.infer_trajectory(fadata, parameters=parameters)  # add parameters when inferring trajectory

    # plot
    cfe.plot.plot_trajectory(fadata, basis="umap", color=["milestone"], save=save_fig)
...
def benchmark(
    ...
    for method_name in method_list:
        ...
        method = cfe.method.FateMethod(method_name=method_name, backend_name="conda")
        method.infer_trajectory(fadata, parameters=parameters)  # add parameters when inferring trajectory
        # plot
        fig_file = f"{save_fig_dir}/{method.method_name}.jpg"
        cfe.plot.plot_trajectory(fadata, basis="umap", color=["milestone"], save=fig_file)

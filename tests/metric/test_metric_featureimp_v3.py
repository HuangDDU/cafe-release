...
@pytest.fixture
def fi_method_tiny():
    return fi_ranger_rf_tiny(
        num_trees=50,
        num_variables_per_split=2,
...
def test_calculate_overall_feature_importance(fadata_dataset, fi_method_tiny):
    overall = calculate_overall_feature_importance(
        fadata_dataset,
...
def make_toy_fadata():
    ...
    expr = pd.DataFrame({
        'G1': m1 + np.random.normal(0, 0.01, 20),
        'G2': m2 + np.random.normal(0, 0.01, 20),
        'G3': np.random.rand(20)
    }, index=cells)

    mp = pd.DataFrame({
        'cell_id': cells * 2,
        'milestone_id': ['M1'] * 20 + ['M2'] * 20,
        'percentage': np.concatenate([m1, m2])
    })
    mn = pd.DataFrame({'from':['M1'],'to':['M2'],'length':[1],'directed':[True]})
...
def test_featureimp_cor_midrange():
    ...
    mw2 = MilestoneWrapper(
        milestone_network=fa.milestone_wrapper.milestone_network,
        milestone_percentages=pred_mp
    )
    ...
    cor = calculate_featureimp_cor(
        fa, ref_model='ref', pred_model='pred',
        fi_method=fi_ranger_rf_tiny(num_trees=500)
    )

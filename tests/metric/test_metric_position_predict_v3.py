...
def make_fadata(mp_ref, mp_pred):
    ...
    fadata = FateAnnData(X=np.empty((len(CELL_IDS),1)),
                         obs=obs,
...
def test_same_data_RF_and_LM_valid_ranges(fadata_same):
    ...
    out = calculate_position_predict(
        fadata_same, ref_model='ref', pred_model='pred'
    )['summary']
...
def test_diff_data_LM_not_perfect(fadata_diff):
    ...
    out = calculate_position_predict(
        fadata_diff, ref_model='ref', pred_model='pred'
    )['summary']
    assert out['lm_mse'] > 0

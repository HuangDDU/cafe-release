...
from cfe.metric.metric_featureimp import (  # get_expression,; is_wrapper_with_trajectory,
...
def test_calculate_featureimp_cor(fadata_dataset, fadata_prediction, fi_method_tiny):
    cor_dict = calculate_featureimp_cor(
        dataset=fadata_dataset, prediction=fadata_prediction, expression_source="expression", fi_method=fi_method_tiny
    )
...
def test_featureimp_cor_insufficient_cells(fadata_dataset):
    ...
    milestone_network = pd.DataFrame({"from": ["M1"], "to": ["M2"], "length": [1], "directed": [True]})
    milestone_percentages = pd.DataFrame({"cell_id": cell_ids * 2, "milestone_id": ["M1", "M1", "M2", "M2"], "percentage": [0.7, 0.8, 0.3, 0.2]})
    mw = MilestoneWrapper(milestone_network=milestone_network, milestone_percentages=milestone_percentages)
    fadata_pred.milestone_wrapper = mw

    cor_dict = calculate_featureimp_cor(dataset=fadata_dataset, prediction=fadata_pred, expression_source="expression")
...
def test_featureimp_enrichment_insufficient_cells(fadata_dataset):
    ...
    enrich_dict = calculate_featureimp_enrichment(dataset=fadata_dataset, prediction=fadata_pred, expression_source="expression")

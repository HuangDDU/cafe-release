...
def make_fadata(milestone_percentages_ref, milestone_percentages_pred):
    ...
    # 参考模型
    mw_ref = MilestoneWrapper(
        milestone_network=mn,
        milestone_percentages=milestone_percentages_ref
    )
    fadata.model_name = 'ref'
    fadata.milestone_wrapper = mw_ref

    # 预测模型
    mw_pred = MilestoneWrapper(
        milestone_network=mn,
        milestone_percentages=milestone_percentages_pred
    )
...
def test_calculate_position_predict_same(fadata_same):
    ...
    gold = pd.pivot_table(
        ...
        index='cell_id', columns='milestone_id', values='percentage', fill_value=0
    )

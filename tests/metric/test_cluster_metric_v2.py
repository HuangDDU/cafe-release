...
from cfe.metric.cluster_metric_v2 import calculate_mapping_milestones
...
def sample_fadata():
    ...
    mw_ref = MilestoneWrapper(
        milestone_network=ref_network,
        milestone_percentages=ref_percent
    )
    # 添加到 fadata
    ...
    mw_pred = MilestoneWrapper(
        milestone_network=pred_network,
        milestone_percentages=pred_percent
    )

...
def test_subsample():
    ...
    # random subsample
    adata_subsample1 = cafe.preprocess.subsample(adata, n_obs)
    assert adata_subsample1.shape == (n_obs, adata.shape[1])

    # save specific cell
    save_cell_list = ["192", "278", "103"]
    adata_subsample2 = cafe.preprocess.subsample(adata, n_obs, save_cell_list=save_cell_list)
    assert len(list(set(adata_subsample2.obs.index) - set(save_cell_list))) == n_obs - len(save_cell_list)

    # save 1 cell for every cluster
    cluster_key = "lineage"
    adata_subsample3 = cafe.preprocess.subsample(adata, n_obs, save_cluster_key=cluster_key)

...
if_test_raw = False  # change to True when run in 'unitvelo' conda environment


@pytest.mark.run_method
class TestCFUnitvelo:
    def setup_method(self):
        self.method_name = "unitvelo"
    ...
    # pytest -s --tb=long test_cf_unitvelo.py
    @pytest.mark.skipif(not if_test_raw, reason="skip raw test, because it should be in conda environment 'unitvelo'")
    def test_raw(self):
        import sys

        sys.path.append("../../../cafe/method/function")  # prepare relative package file
        from cf_unitvelo import unitvelo

        trajectory_dict = unitvelo(self.adata, **self.parameters)
